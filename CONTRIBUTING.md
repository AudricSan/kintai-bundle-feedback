# Contributing to kintai-bundle-feedback
...
- Namespace root: `kintai\Bundles\Installed\Feedback\` → `src/`
...
- Persistence goes through `FeedbackRepositoryInterface` (bound in
  `FeedbackBundle::registerServices()`) — controllers never touch storage
  directly
- Comments in code are written in French; everything else (commit messages,
  PR descriptions, docs) in English
- Never use inline `style="..."` in `Views/feedbacks.php` — it's rendered
